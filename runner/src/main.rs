...
mod run;
mod karesz;
use karesz::{Karesz, Game, GameActions, Moves};
use std::fs;
...
fn run_multiplayer() {
    ...
    match fs::write("../Program.cs", prepare::create_multi_player_template(&mut v, "", key, round_key)) {
...
fn run_single() {
    // values 
    let mut game = Game { size_x:10, size_y:10, rounds:0, objects:HashMap::new(), players:HashMap::new(), proposed_steps:HashMap::new(), death_row: Vec::new() };
    let mut player = Karesz { id:0, position: (2, 5), rotation:0, steps:Vec::new(), is_moving:false, kills:0 };
    ...
    // Write to file
    match fs::write("./Program.cs", prepare::create_single_player_template(code, key, key)) {
        Ok(_) => println!("Write OK"),
        Err(e) => println!("uh oh: {}", e)
    }
