 ...
 */
 export const RULES:Array<ReplacementRules> = [
    ...
    { std:'out', cmd: Command.turn_right, match: /Fordulj_jobbra\s*\(\s*\)/gm },
    { std:'out', cmd: Command.turn_left, match: /Fordulj_balra\s*\(\s*\)/gm },
    { std:'out', cmd: Command.turn_left, match: /Fordulj\s*\(\s*balra\s*\)/gm },
    { std:'out', cmd: Command.turn_right, match: /Fordulj\s*\(\s*jobbra\s*\)/gm },
    { std:'out', cmd: Command.pick_up_rock, match: /Vegyél_fel_egy_kavicsot\s*\(\s*\)/gm },
    { std:'out', cmd: `"${Command.place_rock} 2"`, match: /Tegyél_le_egy_kavicsot\s*\(\s*\)/gm },
    { std:'out', cmd: `"${Command.place_rock}"+:x:`, match: /Tegyél_le_egy_kavicsot\s*\(.*\)/gm, x:true },
    { std:'in', cmd: `"${Command.check_direction}","0"`, match: /Északra_néz\s*\(\s*\)/gm },
    { std:'in', cmd: `"${Command.check_direction}","2"`, match: /Délre_néz\s*\(\s*\)/gm },
    { std:'in', cmd: `"${Command.check_direction}","3"`, match: /Keletre_néz\s*\(\s*\)/gm },
    { std:'in', cmd: `"${Command.check_direction}","1"`, match: /Nyugatra_néz\s*\(\s*\)/gm },
    { std:'in', cmd: Command.check_direction, match: /Merre_néz\s*\(\s*\)/gm },
    { std:'in', cmd: `"${Command.check_under}","1"`, match: /Van_e_itt_kavics\s*\(\s*\)/gm },
    { std:'in', cmd: Command.check_field, match: /Mi_van_alattam\s*\(\s*\)/gm },
    { std:'in', cmd: `"${Command.check_wall}","1"`, match: /Van_e_előttem_fal\s*\(\s*\)/gm },
    { std:'in', cmd: `"${Command.check_bounds}","1"`, match: /Kilépek_e_a_pályáról\s*\(\s*\)/gm },
    { std:'out', cmd: `"${Command.turn_direction}"+:x:`, match: /Fordulj\s*\(.*\)/gm, x:true },
