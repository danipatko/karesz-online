...
export class Command {
    ...
    public run(commandOptions: CommandOptions = {}): Command {
        ...
        if (this.isLinux && commandOptions.limit)
            this.args = prlimit(this.args, {
                nproc: commandOptions.limit.nproc,
                stack: commandOptions.limit.stack,
                cputime: commandOptions.limit.cputime,
            });
        ...
        if (this.process === null || this.process.stdout == null)
            throw new Error(
                `An error occured when spawning '${this.args.join(' ')}'.`
            );

        // set error handlers
        this.process.stdout?.addListener('error', (e) =>
            this.errorHandler(e, (x) => this.kill(x))
        );
        this.process.addListener('error', (e) =>
            this.errorHandler(e, (x) => this.kill(x))
        );
        this.process.stderr?.addListener('error', (e) =>
            this.errorHandler(e, (x) => this.kill(x))
        );
        this.process.stderr?.addListener('data', (e) =>
            this.errorHandler(e, (x) => this.kill(x))
        );
        // set data handlers
        this.process.stdout?.addListener('data', (s) =>
            this.dataHandler(
                s,
                (x) => this.write(x),
                (x) => this.kill(x)
        ...
        this.process.addListener('message', (s) =>
            this.dataHandler(
                s.toString(),
                (x) => this.write(x),
                (x) => this.kill(x)
        ...
        this.process.stdout?.setEncoding(
            commandOptions.std?.encoding || 'utf-8'
        );
        if (commandOptions.std?.write)
     ...
     */
    public onError(
        handler: (err: Error, kill: (signal: NodeJS.Signals) => void) => void
    ): Command {
    ...
    public write(data: string): void {
        if (!this || this.process === undefined)
            throw new Error(
                `Unable to write to stdin: process was never spawned`
            );
    ...
    public async runAsync(commandOptions: CommandOptions = {}): Promise<any> {
        return new Promise<any>((res) => {
            this.onExit((code, signal) => res({ code, signal })).run(
                commandOptions
            );
        });
    }
